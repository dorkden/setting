...
class MultiMA_TSL(IStrategy):
    ...
    # Multi Offset
    base_nb_candles_sell = IntParameter(5, 80, default=20, load=True, space='sell', optimize=True)

    base_nb_candles_buy_sma = IntParameter(5, 80, default=20, load=True, space='buy', optimize=True)
    low_offset_sma = DecimalParameter(0.9, 0.99, default=0.958, load=True, space='buy', optimize=True)
    rsi_buy_sma = IntParameter(30, 70, default=61, space='buy', optimize=True)

    base_nb_candles_buy_ema = IntParameter(5, 80, default=20, load=True, space='buy', optimize=True)
    low_offset_ema = DecimalParameter(0.9, 0.99, default=0.958, load=True, space='buy', optimize=True)
    high_offset_ema = DecimalParameter(0.99, 1.1, default=1.012, load=True, space='sell', optimize=True)
    rsi_buy_ema = IntParameter(30, 70, default=61, space='buy', optimize=True)
    ...
    # Protection
    ewo_low = DecimalParameter(-20.0, -8.0, default=-20.0, load=True, space='buy', optimize=True)
    ewo_high = DecimalParameter(2.0, 12.0, default=6.0, load=True, space='buy', optimize=True)
    fast_ewo = IntParameter(10, 50, default=50, load=True, space='buy', optimize=True)
    slow_ewo = IntParameter(100, 200, default=200, load=True, space='buy', optimize=True)

    # stoploss sharp dip filter
    sl_filter_candles = IntParameter(2, 10, default=5, space='sell', optimize=True, load=True)
    sl_filter_offset = DecimalParameter(0.960, 0.999, default=0.989, decimals=3, space='sell', optimize=True,
                                        load=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        if not self.config['runmode'].value == 'hyperopt':
            dataframe['ma_sl_filter_offset'] = ta.EMA(dataframe, timeperiod=int(
                self.sl_filter_candles.value)) * self.sl_filter_offset.value
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['sma_offset_buy'] = ta.SMA(dataframe,
                                             int(self.base_nb_candles_buy_sma.value)) * self.low_offset_sma.value
        ...
        dataframe['ema_offset_sell'] = ta.EMA(dataframe,
